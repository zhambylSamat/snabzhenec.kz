<?php
	...
	if(isset($_POST['remove_company'])){
		try {
			$stmt = $conn->prepare("DELETE FROM company WHERE company_num = :company_num");
			$stmt->bindParam(':company_num',$_POST['data_num'],PDO::PARAM_STR);

			$stmt->execute();

			$_SESSION['n'] = 'true';
			$data['result'] = $result;
		    header('location:index.php?remove_company=true');	
		} catch (PDOException $e) {
			echo "Error : ".$e->getMessage()." !!!";
		}
	}
